...
- Several bottlenecks to outbound HTTP requests have been identied and removed

- `String::length` is now `O(1)` instead of `O(N)`

- JSON output are now significantly faster
...
Dark is now standards compliant, and uses a different formatting.
...
- Standardized our production deployment process using a tool we wrong called
